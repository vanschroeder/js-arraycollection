...
{_} = require('underscore') if typeof exports != 'undefined'
'use strict'
...
class global.ArrayCollection
  ...
  clearSource : ->
    @removeAll()
  # adds all passed list members to source
  addAll : (list)->
  ...
  getItemIndex : (itm)->
    @__list.indexOf itm
  # triggers a collection change -- don't call directly
  collectionChanged : (operation, items) ->
    data = {}
    data[operation] = if _.isArray items then items else [items]
    @__updated = true
    @__serial = @__serialize @__list
